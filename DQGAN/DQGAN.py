...
def qnnTrainingGen(qnnArch, qnnArchGen, currentUnitaries, storedStates, lda, ep, trainingData):
    ...
    for l in range(1, len(qnnArch)):
        ...
        for j in range(numOutputQubits):
            newUnitaries[l][j] = (
                        makeUpdateMatrixTensoredGen(qnnArch, qnnArchGen, currentUnitaries, storedStates, lda, ep, l, j,
                                                    trainingData) * currentUnitaries[l][j])
    currentUnitaries = newUnitaries
...
def makeUpdateMatrixTensoredDis(qnnArch, qnnArchGen, unitaries, lda, ep, storedStates, storedStatesDis, l, j,
                                trainingData):
    numInputQubits = qnnArch[l - 1]
    numOutputQubits = qnnArch[l]

    res = makeUpdateMatrixDis(qnnArch, qnnArchGen, unitaries, lda, ep, storedStates, storedStatesDis, l, j,
                              trainingData)
    if numOutputQubits - 1 != 0:
        res = qt.tensor(res, tensoredId(numOutputQubits - 1))
    return swappedOp(res, numInputQubits, numInputQubits + j)
...
def qnnTrainingDis(qnnArch, qnnArchGen, currentUnitaries, storedStates, storedStatesDis, lda, ep, trainingData):
    ...
    for l in range(1, len(qnnArch)):
        ...
        # Loop over perceptrons
        for j in range(numOutputQubits):
            newUnitaries[l][j] = (makeUpdateMatrixTensoredDis(qnnArch, qnnArchGen, currentUnitaries, storedStates,
                                                              storedStatesDis, lda, ep, l, j, trainingData) *
...
def QGANStatistics(blochPossible, fileName, trainingRoundNr, trainingData, numTestingData, numTest, numTrainingData,
                   numIn, qnnArchGen, networkUnitariesGen, testingData, indexDataTest, indexDataTrain, qnnArchGenString,
                   qnnArchDisString):
...
def QGAN(qnnArch, lastLayerOfGen, data, lda, ep, trainingRounds, trainingRoundsGen,
    ...
    numIn = qnnArch[0]
    numOutGen = qnnArch[
        lastLayerOfGen - 1]  # number of qubits of output of generator = number of qubits input of discriminator
    ...
    for round in range(trainingRounds):
        ...
        for k in range(trainingRoundsGen):
            ...
            # training generator
            newNetworkUnitaries = qnnTrainingGen(qnnArch, qnnArchGen, networkUnitaries, storedStates, lda, ep,
                                                 trainingData)  # TODO: trained all unitaries, not efficient
            networkUnitariesGen = newNetworkUnitaries[:lastLayerOfGen]
            networkUnitaries = networkUnitariesGen + networkUnitariesDis[
                                                     1:]  # remove emtpy list at beginning for structure reasons
        ...
        for k in range(trainingRoundsDis):
            ...
            newNetworkUnitaries = qnnTrainingDis(qnnArch, qnnArchGen, networkUnitaries, storedStates, storedStatesDis,
                                                 lda, ep, trainingData)
        ...
        if (trainingRoundNr) % statRounds == 0:
            ...
            QGANStatistics(blochPossible, fileName, trainingRoundNr, trainingData, numTestingData, numTest,
                           numTrainingData, numIn, qnnArchGen, networkUnitariesGen, testingData, indexDataTest,
                           indexDataTrain, qnnArchGenString, qnnArchDisString)
    ...
    plt.close()
    df = pd.DataFrame(
        {'step times epsilon': plotlist[0], 'costFunctionGen': plotlist[1], 'costFunctionDis': plotlist[2],
         'costFunctionTest': plotlist[3], 'costHilbert': plotlist[4]})
...
def QGANMeans(qnnArch, lastLayerOfGen, data, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis,
              numTestingData, numTest, statRounds, trainingDataName, meanNum):
    for i in range(1, meanNum + 1):
        meanName = trainingDataName + '_plot' + str(i)
        print(meanName)
        QGAN(qnnArch, lastLayerOfGen, data, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis,
             numTestingData, numTest, statRounds, meanName)
    ...
    for trainingRoundNr in range(1, trainingRounds + 1):
        if (trainingRoundNr) % statRounds == 0:
            fileName = 'QGAN_' + str(numData) + 'data' + str(numTrainingData) + 'sv_' + str(
                numTest) + 'statData_' + str(
                numTest) + 'statData_' + qnnArchGenString + 'networkGen_' + qnnArchDisString + 'networkDis' + '_lda' + str(
                lda).replace('.', 'i') + '_ep' + str(ep).replace('.', 'i') + '_rounds' + str(
                trainingRoundNr) + '_roundsGen' + str(trainingRoundsGen) + '_roundsDis' + str(
                trainingRoundsDis) + '_' + trainingDataName
...
def QGANStatisticsVsLine(blochPossible, fileName, trainingRoundNr, trainingData, numTestingData, numTest,
                         numTrainingData, numIn, qnnArchGen, networkUnitariesGen, testingData, indexDataTest,
                         indexDataTrain, qnnArchGenString, qnnArchDisString):
    countOutTest = [0 for i in range(numTestingData)]
    countOutTrain = [0 for i in range(numTrainingData)]
...
def QGANVsLine(qnnArch, lastLayerOfGen, data, lda, ep, trainingRounds, trainingRoundsGen,
    ...
    for round in range(trainingRounds):
        ...
        if (trainingRoundNr) % statRounds == 0:
            print("Statistics of Training Round ", trainingRoundNr)
            QGANStatisticsVsLine(blochPossible, fileName, trainingRoundNr, trainingData, numTestingData, numTest,
                                 numTrainingData, numIn, qnnArchGen, networkUnitariesGen, testingData, indexDataTest,
                                 indexDataTrain, qnnArchGenString, qnnArchDisString)
    plt.scatter(plotlist[0], plotlist[1], label='costFunctionGen', color='orange', s=10)
    plt.scatter(plotlist[0], plotlist[2], label='costFunctionDis', color='green', s=10)
...
def QGANMeansVsLine(qnnArch, lastLayerOfGen, data, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis,
                    numTestingData, numTest, statRounds, trainingDataName, meanNum):
    for i in range(1, meanNum + 1):
        meanName = trainingDataName + '_plot' + str(i)
        print(meanName)
        QGANVsLine(qnnArch, lastLayerOfGen, data, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis,
                   numTestingData, numTest, statRounds, meanName)
...
qnnArch = [1, 1, 1]  # architecture of the network, for example [2,3,2] or [4,1]
lastLayerGan = 2  # number of layers of generator
lda = 1  # lambda = 1/learning rate
ep = 0.01  # epsilon = step size
trainingRounds = 1000
trainingRoundsGen = 1  # rounds generator
trainingRoundsDis = 1  # rounds discriminator
numTestingData = 40  # length testing data set
numTest = 100  # number of states for training
statRounds = 100  # every statRounds there will be a test
shots = 2  # training shots

numData = 50
line1 = line(numData, 1)
QGANMeans(qnnArch, lastLayerGan, line1, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis, numTestingData,
          numTest, statRounds, "line", shots)

clusterCon = connectedClusters(numData, 1)
QGANMeansVsLine(qnnArch, lastLayerGan, clusterCon, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis,
                numTestingData, numTest, statRounds, "conCvsLi", shots)

clusters = clusters(numData, 1)
QGANMeansVsLine(qnnArch, lastLayerGan, clusters, lda, ep, trainingRounds, trainingRoundsGen, trainingRoundsDis,
                numTestingData, numTest, statRounds, "CvsLi", shots)